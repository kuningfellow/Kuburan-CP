...
int dp[16][(1<<16)][2];
int m;
int serc(int x, int mask, int flag){
	if (mask==(1<<m)-1){
		return 0;
	}
	int ret=(1<<29);
	if (dp[x][mask][flag]!=-1){
		return dp[x][mask][flag];
	}
	if (flag==0){
		for (int i=0;i<m;i++){
			ret=min(ret,serc(i,(1<<i),1));
		}
	}
	else{
		...
		for (int i=0;i<cost[x].size();i++){
			if ((mask&(1<<cost[x][i].first))==0){
				ret=min(ret,cost[x][i].second*flag+serc(cost[x][i].first,mask|(1<<cost[x][i].first),1));	
			}
		}
	}
	return dp[x][mask][flag]=ret;
}
int main(){
	int n;
