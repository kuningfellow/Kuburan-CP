...
int main() {
	...
	for (int i = 0; i < eval.size(); i++) {
		if (i == 0) {
			...
			double c = LICHAO::query(root, 0, 1000000, eval[i-1].second);
			LICHAO::update(root, 0, 1000000, eval[i].first, c);
		}
	}
	printf("%lld\n", LICHAO::query(root, 0, 1000000, maxi));
	return 0;
}