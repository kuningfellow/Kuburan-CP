...
void buildlca(int n){
	...
	for (int i=1;i<=p;i++){
		for (int j=0;j<n;j++){
			pts[j].par[i]=pts[pts[j].par[i-1]].par[i-1];
		}
	}
}
int lca(int a, int b, int n){
	int p=getord(n);
	if (pts[a].depth<pts[b].depth){
		int temp=a;
		a=b;
		b=temp;
	}
	int delta=pts[a].depth-pts[b].depth;
	for (int i=0;i<=p;i++){
		if ((delta&(1<<i))>0){
			a=pts[a].par[i];
		}
...
int hldquery(int a, int b, int n){
	...
	if (pts[a].cpar==pts[b].cpar){
		return query(0,n-1,1,pts[a].pos+1,pts[b].pos);
	}
	return max(hldquery(a,pts[pts[b].cpar].par[0],n),query(0,n-1,1,pts[pts[b].cpar].pos,pts[b].pos));
}
int main(){
	int tc;
	scanf("%d",&tc);
	while (tc--){
		...
		while (1<2){
			...
			if (!strcmp(rem,"QUERY")){
				...
				b--;
				int meet=lca(a,b,n);
				printf ("%d\n",max(hldquery(meet,a,n),hldquery(meet,b,n)));
			}
			else{
				scanf("%d%d",&a,&b);
				a--;
