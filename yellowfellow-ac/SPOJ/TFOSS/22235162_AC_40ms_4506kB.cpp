...
namespace GEOM {
  ...
  vector<pair<int,TD> > rotateCalipers(const vector<Pt> &Pts, int mod = 0) {
    ...
    for (int i = 0, n = Pts.size(); i < n; i++, upd = ptr) {
      ...
      for (int j = 0; j < n; j++) {
        if (d <= (tmp = abs(cross(Pts[upd], Pts[(i+1)%n], Pts[i]))) + EPS) {
          d = tmp;
          for (int k = -10; k <= 10; k++) {
            finans = max(finans, (LL)(Pts[i].first-Pts[(upd+k+n)%n].first)*(LL)(Pts[i].first-Pts[(upd+k+n)%n].first) + (LL)(Pts[i].second-Pts[(upd+k+n)%n].second)*(LL)(Pts[i].second-Pts[(upd+k+n)%n].second));
          }
          upd = (upd + 1) % n;
        } else {
          break;
