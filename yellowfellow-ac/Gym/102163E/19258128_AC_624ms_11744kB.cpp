...
namespace ST {
	...
	dat combine(dat L, dat R) {
		dat ret;
		ret.len = L.len + R.len;
		ret.x1_r = L.x1_r + R.x1_r * shifts[L.len][0];
		ret.x1_l = L.x1_l * shifts[R.len][0] + R.x1_l;
		ret.x2_r = L.x2_r + R.x2_r * shifts[L.len][1];
		ret.x2_l = L.x2_l * shifts[R.len][1]+ R.x2_l;
		return ret;
	}
...
int main() {
	...
	for (int i = 1; i < 100005; i++) {
		for (int j = 0; j < 2; j++) {
			shifts[i][j] = shifts[i-1][j] * sigma[j];
		}
	}
	int tc;
	scanf("%d", &tc);
		...
		while (q--) {
			...
			if (type == 1) {
				...
				if (l == r) {
					...
					if (le.x1_l == ri.x1_r && le.x2_l == ri.x2_r) {
						printf("Adnan Wins\n");
					}
					else {
						printf("ARCNCD!\n");
